...
public class DoorControl extends Controller {
	...
	public DoorControl(Hallway hallway, Side side, SimTime period,
			boolean verbose) {
		super("DoorControl"
		...
		networkDesiredDwell = CanMailbox
				.getReadableCanMailbox(MessageDictionary.DESIRED_DWELL_BASE_CAN_ID
		...
		networkDoorMotor = CanMailbox
				.getWriteableCanMailbox(MessageDictionary.DOOR_MOTOR_COMMAND_BASE_CAN_ID
						+ ReplicationComputer.computeReplicationId(hallway,
								side));
		mDoorMotor = new DoorMotorCanPayloadTranslator(networkDoorMotor,
				hallway, side);
	...
	public void timerExpired(Object callbackData) {
		...
		case CLOSED: /* State 3 Closed */
			...
			// chart
			if ((this.isOverweight() && mAtFloor.getCurrentFloor() != -1 && mAtFloor
					.isAtFloor(mAtFloor.getCurrentFloor(), hallway))
					|| (mAtFloor.getCurrentFloor() == mDesiredFloor.getFloor()
							&& (mDesiredFloor.getHallway() == hallway || mDesiredFloor
									.getHallway() == Hallway.BOTH) && (mDriveSpeed
							.getSpeed() == 0 || mDriveSpeed.getDirection() == Direction.STOP))) {
				newState = State.OPENING;
			}
			break;
