 ...
 * 
 * summarize() method reports:
 * 	- Number of times car was overweight
 *  - Amount of time (in seconds) spent in door reversals
 *  - Number of wasted openings
 * 
 */
public class Proj7RuntimeMonitor extends RuntimeMonitor {
    ...
    Stopwatch reversalTime = new Stopwatch();
    ...
    protected String[] summarize() {
        ...
        arr[1] = "Reversal Count = " + reversalTime.accumulatedTime;
        arr[2] = "Wasted Opening Count = " + wastedOpeningCount;
        return arr;
    }
    ...
    private void doorClosed(Hallway hallway) {
        ...
        if (!doorState.anyDoorOpen()) {
            ...
            // This means weight didn't change since last opening door
            // This was a wasted opening
            if (!weightChanged) {
    ...
    private void doorOpened(Hallway hallway) {
    	...
    	// Door opened means reversal is done, stop timing it
        if (reversalTime.isRunning()) {
        	reversalTime.stop();
        }
    }
    
    /**
     * Called once when the doors start reversing
     * @param hallway which door the event pertains to
     */
