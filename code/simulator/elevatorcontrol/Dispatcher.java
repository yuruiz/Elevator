...
public class Dispatcher extends Controller {
	...
	public void timerExpired(Object callbackData) {
		...
		case StopStop:
			...
			desiredHallway = (curFloorCarCall.isValid()) ? curFloorCarCall.hallway
					: Hallway.NONE;
		...
		case StopDown:
			...
			if (targetRequest.isValid()) {
				...
				if (CurrentFloor != -1) {
					CallRequest curFloorCall = mHallCallArray.isCalled(
							CurrentFloor, Direction.DOWN);
			...
			if (mFrontDoorClosed.getBothClosed()
				...
				// #transition DPT.14
					...
					&& !(closestHallCallAbove.isValid() || closestCarCallAbove
							.isValid())) {
				nextState = State.StopStop;
			}
		...
		case UpUp:
			...
			targetRequest = computeTarget(upUpHallCall, closestCarCallAbove,
					Direction.UP);
			if (targetRequest.isValid()) {
				Target = targetRequest.floor;
				desiredHallway = targetRequest.hallway;
		...
		case DownDown:
			...
			targetRequest = computeTarget(downDownHallCall,
					closestCarCallBelow, Direction.DOWN);
			if (targetRequest.isValid()) {
				Target = targetRequest.floor;
				desiredHallway = targetRequest.hallway;
			} else {
		...
		case UpDown:
			...
			CallRequest minDownHallCallAboveTarget = mHallCallArray
					.maxGoingDown(Target - 1, canCommit);
			// Get the next up hall call above the current floor
			CallRequest upHallCallAboveCurFloor = mHallCallArray.minGoingUp(
					previousFloorSeen, canCommit);
			carCallBeforeTarget = mCarCallArray.lowestCallBetween(
					previousFloorSeen, Target, canCommit);
			...
			if (carCallBeforeTarget.isValid()
					|| upHallCallAboveCurFloor.isValid()) {
				// Car call before target or Hall Call in the up direction => Go
				// to UpUp
				nextState = State.UpUp;
				// #transition DPT.17
			} else if (mCarCallArray.closestCallAbove(Target, canCommit)
					.isValid()) {
				nextState = State.UpStop;
				// #transition DPT.1
		...
		case DownUp:
			...
			// Find the minimum hall call below the target
			CallRequest minUpHallCallBelowTarget = mHallCallArray.minGoingUp(
					Target + 1, canCommit);
			// Get the next down hall call below the current floor
			CallRequest downHallCallBelowCurFloor = mHallCallArray
					.maxGoingDown(previousFloorSeen, canCommit);
			carCallBeforeTarget = mCarCallArray.highestCallBetween(
					previousFloorSeen, Target, canCommit);
			...
			// #transition DPT.11
			if (carCallBeforeTarget.isValid()
					|| downHallCallBelowCurFloor.isValid()) {
				nextState = State.DownDown;
				// #transition DPT.16
			} else if (mCarCallArray.closestCallBelow(Target, canCommit)
					.isValid()) {
				nextState = State.DownStop;
				// #transition DPT.1
			} else if (!(mFrontDoorClosed.getBothClosed() && mBackDoorClosed
		...
		if (!(closestCarCall.isValid() || closestHallCall.isValid())) {
			...
			case UP:
				if (closestCarCall.floor < closestHallCall.floor) {
					...
					floor = closestCarCall.floor;
					hallway = CallRequest.union(closestCarCall.hallway,
							closestHallCall.hallway);
				} else {
					floor = closestCarCall.floor;
					hallway = closestCarCall.hallway;
				}
