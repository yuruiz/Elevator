...
public class Dispatcher extends Controller {
	...
	public void timerExpired(Object callbackData) {
		...
		case Initial:
			...
			CurrentDirection = (CurrentFloor == 1) ? Direction.STOP
					: Direction.DOWN;
			...
			if (CurrentDirection == Direction.STOP) {
				...
				log("CurrentDirection is " + CurrentDirection);
			}
			break;
		case StopStop:
			// DONE
			...
			mDesiredFloor.set(Target, DesiredDirection, Hallway.NONE);
			// #transition DPT.6
			if (closestCarCallAbove.isValid()
					|| closestHallCallAbove.isValid()
					|| mHallCallArray.isCalled(CurrentFloor, Direction.UP)
							.isValid()) {
				nextState = State.StopUp;
				// #transition DPT.7
			} else if (closestCarCallBelow.isValid()
					|| closestHallCallBelow.isValid()
					|| mHallCallArray.isCalled(CurrentFloor, Direction.DOWN)
							.isValid()) {
		...
		case StopDown:
			...
			targetRequest = computeTarget(closestCarCallBelow,
					closestHallCallBelow, DesiredDirection);
			if (targetRequest.isValid()) {
				Target = targetRequest.floor;
				desiredHallway = targetRequest.hallway;
			} else {
				throw new RuntimeException(
						"Got to state StopDown without any down calls.");
			}
		...
		case StopUp:
			...
			targetRequest = computeTarget(closestCarCallAbove,
					closestHallCallAbove, DesiredDirection);
			if (targetRequest.isValid()) {
				Target = targetRequest.floor;
				desiredHallway = targetRequest.hallway;
			} else {
				throw new RuntimeException(
						"Got to state StopUp without any up calls.");
			}
		...
		case UpStop:
			...
			DesiredDirection = Direction.STOP;
			targetRequest = computeTarget(closestCarCallAbove,
					closestHallCallAbove, CurrentDirection);
			if (targetRequest.isValid()) {
				Target = targetRequest.floor;
				desiredHallway = targetRequest.hallway;
			} else {
				throw new RuntimeException(
						"Got to state UpStop without any up calls.");
			}

			// Check for calls above Target
			CallRequest nextCarCallAbove = mCarCallArray.closestCallAbove(
					Target, canCommit);
			CallRequest nextHallCallAbove = mHallCallArray.closestCallAbove(
					Target, canCommit);

			// #transition DPT.3
			if ((closestHallCallAbove.isValid() && closestHallCallAbove.direction == Direction.UP)
					|| (nextCarCallAbove.isValid() || nextHallCallAbove
							.isValid())) {
		...
		case DownStop:
			...
			DesiredDirection = Direction.STOP;
			targetRequest = computeTarget(closestCarCallBelow,
					closestHallCallBelow, CurrentDirection);
			if (targetRequest.isValid()) {
				Target = targetRequest.floor;
				desiredHallway = targetRequest.hallway;
			} else {
				throw new RuntimeException(
						"Got to state DownStop without any down calls.");
			}

			// Check for calls below Target
			CallRequest nextCarCallBelow = mCarCallArray.closestCallBelow(
					Target, canCommit);
			CallRequest nextHallCallBelow = mHallCallArray.closestCallBelow(
					Target, canCommit);

			// #transition DPT.9
			if ((closestHallCallBelow.isValid() && closestHallCallBelow.direction == Direction.DOWN)
					|| (nextCarCallBelow.isValid() || nextHallCallBelow
							.isValid())) {
		...
		case UpUp:
			...
			CallRequest upUpHallCall = mHallCallArray
					.closestCallAboveInDirection(previousFloorSeen,
							Direction.UP, canCommit);
			if (upUpHallCall.isValid() && closestCarCallAbove.isValid()) {
				if (upUpHallCall.floor < closestCarCallAbove.floor) {
					Target = upUpHallCall.floor;
					desiredHallway = upUpHallCall.hallway;
				} else {
					Target = closestCarCallAbove.floor;
					desiredHallway = closestCarCallAbove.hallway;
				}
			}
			mDesiredFloor.set(Target, DesiredDirection, desiredHallway);
			// #transition DPT.1
		...
		case DownDown:
			...
			CurrentDirection = Direction.DOWN;
			DesiredDirection = Direction.DOWN;
			CallRequest downDownHallCall = mHallCallArray
					.closestCallBelowInDirection(previousFloorSeen,
							Direction.DOWN, canCommit);
			if (downDownHallCall.isValid() && closestCarCallBelow.isValid()) {
				if (downDownHallCall.floor > closestCarCallBelow.floor) {
					Target = downDownHallCall.floor;
					desiredHallway = downDownHallCall.hallway;
				} else {
					Target = closestCarCallAbove.floor;
					desiredHallway = closestCarCallAbove.hallway;
				}
		...
		case UpDown:
			...
			if (targetRequest.isValid()) {
				...
				throw new RuntimeException(
						"Got to state UpDown without any up calls.");
			}

			CallRequest upDownHallCall = mHallCallArray
					.closestCallAboveInDirection(previousFloorSeen,
							Direction.DOWN, canCommit);
			if (upDownHallCall.isValid() && closestCarCallAbove.isValid()) {
				if (upDownHallCall.floor < closestCarCallAbove.floor) {
					Target = upDownHallCall.floor;
					desiredHallway = upDownHallCall.hallway;
				} else {
					Target = closestCarCallAbove.floor;
					desiredHallway = closestCarCallAbove.hallway;
				}
			...
			CallRequest nextCarCallUp = mCarCallArray.closestCallAbove(Target,
					canCommit);
			CallRequest nextHallCallUp = mHallCallArray.closestCallAbove(
					Target, canCommit);
			// #transition DPT.5
			if ((closestHallCallAbove.isValid() && closestHallCallAbove.direction == Direction.UP)
		...
		case DownUp:
			...
			CallRequest downUpHallCall = mHallCallArray
					.closestCallAboveInDirection(previousFloorSeen,
							Direction.DOWN, canCommit);
			if (downUpHallCall.isValid() && closestCarCallAbove.isValid()) {
				if (downUpHallCall.floor < closestCarCallAbove.floor) {
					Target = downUpHallCall.floor;
					desiredHallway = downUpHallCall.hallway;
				} else {
					Target = closestCarCallAbove.floor;
					desiredHallway = closestCarCallAbove.hallway;
				}
			...
			CallRequest nextCarCallDown = mCarCallArray.closestCallBelow(
					Target, canCommit);
			CallRequest nextHallCallDown = mHallCallArray.closestCallBelow(
					Target, canCommit);
			// #transition DPT.11
			if ((closestHallCallBelow.isValid() && closestHallCallBelow.direction == Direction.DOWN)
		...
		// #transition DPT.12
		if (CurrentFloor == -1
				&& !(mFrontDoorClosed.getBothClosed() && mBackDoorClosed
						.getBothClosed())) {
			log("Transition from " + currentState + " --> " + nextState);
	...
	void updateCommitPoints() {
		...
		// |x| = vi^2/(2*a)
		double stoppingDistance = (currSpeed * currSpeed)
				/ (2 * DriveObject.Acceleration);
		...
		case UP:
			...
			nearestFloor = (int) Math.ceil(stoppingPoint
					/ this.mmDistBetweenFloors) + 1;
		...
		case DOWN:
			...
			stoppingPoint = currPos - stoppingDistance;
			nearestFloor = (int) Math.floor(stoppingPoint
					/ this.mmDistBetweenFloors) + 1;
